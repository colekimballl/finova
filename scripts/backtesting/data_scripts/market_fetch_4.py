...
class MarketDataFetcher:
    ...
    def save_data(self, df: pd.DataFrame, symbol: str, market_type: str, timeframe: str):
        ...
        try:
            ...
            # Save file (without timestamp for clean overwriting)
    ...
    try:
        ...
        for market in ['stocks', 'crypto']:
            ...
            if os.path.exists(market_path):
                for symbol in sorted(os.listdir(market_path)):
                    ...
                    if os.path.isdir(symbol_path):
                        ...
                        if files:
                            ...
                            for file in sorted(files):
                                ...
                                timeframe = file.split('_')[1].replace('.parquet', '')
                                date_range = f"{df['date'].min():%Y-%m-%d} to {df['date'].max():%Y-%m-%d}"
                                print(f"  {timeframe:6} : {len(df):6,} rows, {file_size:6.2f}MB | {date_range}")
                                success_count += 1
