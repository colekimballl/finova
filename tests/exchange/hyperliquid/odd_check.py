...
class MarketAnalyzer:
    def __init__(self, symbols=None):
        ...
        # Allow passing a custom list of symbols for testing
        self.symbols = symbols if symbols else ["BTC", "ETH", "ADA", "SOL", "DOGE", "XRP", "LTC", "BNB"]
    ...
    def fetch_historical_data(self, symbol, lookback_hours=72):
        ...
        logging.debug(f"Fetching trades for {symbol} with lookback_hours={lookback_hours}")
        try:
            response = requests.post(url, headers=headers, json=data)
            logging.debug(f"Received response for {symbol}: Status Code {response.status_code}")
            ...
            for symbol in self.symbols:
                try:
                    ...
                    try:
                        ...
                        logging.debug(f"Received order book response for {symbol}: Status Code {l2_response.status_code}")
                        ...
                        logging.exception(f"Exception occurred while fetching order book for {symbol}: {e}")
                        current_price = np.nan
...
if __name__ == "__main__":
    # For initial testing, you can set symbols to a single symbol like ["BTC"]
    analyzer = MarketAnalyzer(symbols=["BTC"])  # Change to None or desired list as needed
