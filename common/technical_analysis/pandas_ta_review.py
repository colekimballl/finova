...
class TechnicalAnalysisPandasTA:
    ...
    def calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(
                "Pandas_TA technical indicator calculations completed successfully."
            )
            return df
    ...
    def _calculate_pandas_ta_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if stoch_k_col in stoch.columns and stoch_d_col in stoch.columns:
            ...
            logger.warning(
                "Stochastic Oscillator columns not found in Pandas_TA output."
            )
        ...
        stats = {
            ...
            "Technical Indicators": {
                "Current Pandas_TA RSI": (
                    f"{df['rsi_pt_14'].iloc[-1]:.2f}"
                    if "rsi_pt_14" in df.columns
                    else "N/A"
                )
            },
            "Volume Statistics": {
...
def display_wide_chart(
    df: pd.DataFrame, price_cols: List[str], pandas_ta_cols: List[str]
):
...
def main():
    try:
        cprint(
            "\n=== Starting Pandas_TA Technical Analysis ===",
            "white",
            "on_blue",
            attrs=["bold"],
        )
        ...
        cprint(
            "\n=== Pandas_TA Technical Analysis Complete ===",
            "white",
            "on_blue",
            attrs=["bold"],
        )
        ...
        cprint(
            "An error occurred during the technical analysis. Check the logs for more details.",
            "red",
            "on_white",
            attrs=["bold"],
        )
