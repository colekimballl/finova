...
log_file = (
    log_dir / f'technical_analysis_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
)
...
class TechnicalAnalysis:
    ...
    def _default_config() -> Dict[str, Any]:
        ...
        return {
            ...
            "volume_indicators": {"mfi_period": 14},
    ...
    def setup_environment(self) -> None:
        ...
        try:
            logger.info(
                f"Pandas_TA Version: {ta_pt.__version__}"
            )  # Log Pandas_TA version
    ...
    def _check_data_quality(self, df: pd.DataFrame) -> None:
        ...
        if null_counts.any():
            logger.warning(
                f"Found null values in columns: \n{null_counts[null_counts > 0]}"
            )
    ...
    def _calculate_ta_lib_indicators(self, df: pd.DataFrame) -> None:
        ...
        df["willr"] = ta.WILLR(
            df["High"], df["Low"], df["Close"], timeperiod=c["willr_period"]
        )

        c = self.config["volatility_indicators"]
        df["atr"] = ta.ATR(
            df["High"], df["Low"], df["Close"], timeperiod=c["atr_period"]
        )
        ...
        df["mfi"] = ta.MFI(
            df["High"], df["Low"], df["Close"], df["Volume"], timeperiod=c["mfi_period"]
        )
    ...
    def _calculate_pandas_ta_indicators(self, df: pd.DataFrame) -> None:
        ...
        stoch = ta_pt.stoch(
            df["High"], df["Low"], df["Close"], k=stoch_k_length, d=stoch_d_length
        )
    ...
    def generate_summary_statistics(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if rsi_pt_col in df.columns:
            ...
            logger.warning(
                f"Pandas_TA RSI column '{rsi_pt_col}' not found in DataFrame."
            )
        ...
        stats = {
            ...
            "Technical Indicators": {
                ...
                "Current Pandas_TA RSI": (
                    f"{current_pandas_ta_rsi:.2f}"
                    if not pd.isna(current_pandas_ta_rsi)
                    else "N/A"
                ),
            },
            "Volume Statistics": {
...
def display_wide_chart(
    df: pd.DataFrame,
    price_cols: List[str],
    indicator_cols: List[str],
    pandas_ta_cols: List[str],
):
...
def main():
    try:
        ...
        indicator_cols = [
            ...
            "mom",
            "roc",
            "willr",  # TA-Lib Momentum
            "atr",
            "upperband",
            "middleband",
            "lowerband",  # TA-Lib Volatility
            "obv",
            "mfi",  # TA-Lib Volume
        ]
        ...
        pandas_ta_cols = [
            "sma_pt_10",
            "ema_pt_10",
            "rsi_pt_14",  # Pandas_TA SMA, EMA, RSI
            "stoch_k",
            "stoch_d",  # Pandas_TA Stochastic Oscillator
        ]
        ...
        cprint(
            ...
            "green" if trend_strength == "STRONG" else "yellow",
            "on_blue",
        )
        ...
        if len(df) >= 2:
            if (
                df["sma"].iloc[-2] < df["ema"].iloc[-2]
                and df["sma"].iloc[-1] > df["ema"].iloc[-1]
            ):
            ...
            logger.warning(
                "Not enough data points to calculate Moving Average Crossovers."
            )
        ...
        if (
            "macd" in df.columns
            and "macdsignal" in df.columns
            and not pd.isna(df["macd"].iloc[-1])
            and not pd.isna(df["macdsignal"].iloc[-1])
        ):
            macd = df["macd"].iloc[-1]
        ...
        if not pd.isna(macd) and not pd.isna(macd_sig):
            cprint(
                f"MACD: {macd:.2f}", "green" if macd > macd_sig else "red", "on_blue"
            )
        else:
        ...
        if "Volume" in df.columns and not pd.isna(df["Volume"].iloc[-1]):
            ...
            vol_status = (
                ...
                else "LOW" if curr_vol < vol_avg * 0.5 else "NORMAL"
            )
        ...
        if (
            ...
            and (not pd.isna(macd) and not pd.isna(macd_sig) and macd > macd_sig)
        ...
        # Caution Conditions
        if vol_status == "HIGH" or (
            not pd.isna(price_change) and abs(price_change) > 5
        ):
            caution_reasons = []
            if vol_status == "HIGH":
